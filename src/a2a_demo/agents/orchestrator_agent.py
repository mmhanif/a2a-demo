...
class OrchestratorAgent(BaseAgent):
    ...
    def handle_task(self, task_id: str, message: TaskMessage) -> str:
        ...
        if (
            "list agents" in content
            or "discover agents" in content
            or "what agents" in content
        ):
    ...
    def _orchestrate_task(self, content: str) -> str:
        ...
        if any(
            word in content
            for word in ["calculate", "solve", "+", "-", "*", "/", "=", "equation"]
        ):
        ...
        if any(
            word in content for word in ["translate", "spanish", "french", "german"]
        ):
        ...
        try:
            ...
            message_response = requests.post(
                ...
                json=JSONRPCRequest(
                    ...
                    params={"task_id": task_id, "role": "user", "content": content},
                    id=2,
                ).to_dict(),
                headers={"Content-Type": "application/json"},
                timeout=5,
