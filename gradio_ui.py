...
def chat_handler(message: str, history: List) -> Tuple[List, str]:
    """Handle chat messages using OpenAI-style message format."""
...
def create_ui():
    ...
    with gr.Blocks(title="A2A Agent Demo", theme=gr.themes.Soft()) as demo:
        ...
        gr.Markdown(
            "Manage agents and interact with the orchestrator to coordinate tasks across specialized agents."
        )
        ...
        with gr.Tabs():
            ...
            with gr.Tab("💬 Orchestrator Chat"):
                ...
                gr.Markdown(
                    "The orchestrator will route your requests to the appropriate specialized agents."
                )
            ...
            # Tab 2: Agent Administration
            with gr.Tab("⚙️ Agent Administration"):
                gr.Markdown("## Agent Management")
                gr.Markdown("View and control the status of all available agents.")
        ...
        send_btn.click(
            chat_handler, inputs=[msg_input, chatbot], outputs=[chatbot, msg_input]
        )
        ...
        refresh_log_btn.click(refresh_log, outputs=[interaction_log])

        clear_log_btn.click(clear_log_handler, outputs=[interaction_log])

        # Event handlers for Agent Administration tab
        start_btn.click(
            start_agent_handler,
        ...
        refresh_btn.click(refresh_status, outputs=[agent_status])

        # Auto-refresh on load
        demo.load(
            lambda: (
                create_agent_status_display(),
                agent_manager.get_interaction_log(),
            ),
